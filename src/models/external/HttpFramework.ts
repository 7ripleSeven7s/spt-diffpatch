...
export const Listen = (basePath: string) =>
{
    return <T extends { new(...args: any[]): any; }>(Base: T): T =>
    {
        ...
        return class extends Base
        {
            ...
            constructor(...args: any[])
            {
                ...
                /*x if (!this.handlers[httpMethod])
                    ...
                    if (this[handlerName] !== undefined && typeof this[handlerName] === "function")
                    {
                        if (!path || path === "")
                        {
                            //x this.handlers[httpMethod][`/${basePath}`] = this[handlerName];
                        }
                        //x this.handlers[httpMethod][`/${basePath}/${path}`] = this[handlerName];
                    }
                }
            ...
            {
                //x const routesHandles = this.handlers[req.method];

                return Object.keys(this.handlers).some((meth) => meth === req.method)
                    //x && Object.keys(routesHandles).some((route) => (new RegExp(route)).test(req.url));
            };
            ...
            handle = (sessionID: string, req: IncomingMessage, resp: ServerResponse): void =>
            {
                ...
                //x const routesHandles = this.handlers[req.method];
                const route = req.url;
                //x const routes = Object.keys(routesHandles);
                //x routes.sort((routeA, routeB) => routeB.length - routeA.length);

                // Filter to select valid routes but only use the first element since it's the most precise
                //x const validRoutes = routes.filter((handlerKey) => (new RegExp(handlerKey)).test(route));
                /*x if (validRoutes.length > 0)
                {
